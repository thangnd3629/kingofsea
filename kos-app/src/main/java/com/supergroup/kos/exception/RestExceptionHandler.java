 ...
 */
@RestControllerAdvice(basePackages = { "com.supergroup.kos.api" })
@Slf4j
public class RestExceptionHandler {
    ...
    public ResponseEntity<?> effectIsActivatedException(EffectIsActivatedException ex) {
        var res = new EffectIsActivatedExceptionResponse()
                .setStatus(Map.of("code", ex.getCode(), "message", ex.getMessage()))
                .setTimestamp(LocalDateTime.now());
        return ResponseEntity.badRequest().body(res);
    }

    @ExceptionHandler(TechRequirementException.class)
    public ResponseEntity<KOSExceptionResponse> techRequirementException(TechRequirementException ex) {
        return new ResponseEntity<>(new KOSExceptionResponse()
                                            .status(ex.getCode())
                                            .data(new TechnologyDTO()
                                                          .setType(ex.getTechnology().getTechnologyType())
                                                          .setCode(ex.getTechnology().getCode())
                                                          .setName(ex.getTechnology().getName()))
                                            .timestamp(LocalDateTime.now()),
                                    HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<KOSExceptionResponse> handleException(Exception exception) {
        if (exception instanceof KOSException) {
            ...
            var errorResponse = new KOSExceptionResponse()
                    .status(ErrorCode.SERVER_ERROR)
                    .timestamp(LocalDateTime.now());
            if (Objects.nonNull(exception.getMessage())) {
                errorResponse.data(exception.getMessage());
            }
