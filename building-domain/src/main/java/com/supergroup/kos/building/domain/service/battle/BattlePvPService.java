...
@AllArgsConstructor
@Slf4j
public class BattlePvPService implements BattleHandler {
    ...
    public Battle startBattle(Battle battle, SeaActivity attacker, SeaElement battleField) {
        ...
        BattleTimeConfigDetail config = kosConfigService.getBattleTimeConfig().getAttack();
        ...
        if (Objects.isNull(shipLineUpAttacker) || Objects.isNull(shipLineUpAttacker.getMotherShip())) {
            onBattleEnded(battle, FactionType.DEFENDER);
            return null;
        }
        BattleProfile battleProfileAttacker = addShipLineupForBattle(battle.getAttacker(), attacker, now);
        BattleProfile battleProfileDefender = addShipLineupForBattle(battle.getDefender(), null, now);
    ...
    private BattleProfile addShipLineupForBattle(BattleProfile battleProfile, SeaActivity activity, LocalDateTime now) {
        if (Objects.nonNull(activity)) {
            battleProfile.setShipLineUps(updateShipLineupForBattle(List.of(activity.getLineUp()), battleProfile, now));
            return battleProfileService.save(battleProfile);
        } else {
            if (BattleType.ATTACK.equals(battleProfile.getBattle().getBattleType())) {
                return createShipLineUpWhenBattleForBase(battleProfile, now);
            } else if (BattleType.OCCUPY.equals(battleProfile.getBattle().getBattleType())) {
                if (isDisputeOccupied(battleProfile.getBattle())) {
                    log.info("Battle:{}, Dispute Occupied, ", battleProfile.getBattle().getId());
                    List<SeaActivity> seaActivities = seaActivityService.getListOccupyInElement(battleProfile.getBattle().getBattleField().getId());
                    List<ShipLineUp> shipLineUps = seaActivities.stream().filter(
                            s -> s.getKosProfile().getId().equals(battleProfile.getKosProfile().getId())).map(s -> s.getLineUp()).collect(
                            Collectors.toList());
                    battleProfile.setShipLineUps(updateShipLineupForBattle(shipLineUps, battleProfile, now));
                    return battleProfileService.save(battleProfile);
                } else {
                    return createShipLineUpWhenBattleForBase(battleProfile, now);
                }
            } else {
                log.info("Battle error : {} start battle: add ship lineUp for battle error", battleProfile.getBattle().getId());
                throw KOSException.of(ErrorCode.BAD_REQUEST_ERROR);
            }
        }
    }

    private List<ShipLineUp> updateShipLineupForBattle(List<ShipLineUp> shipLineUps, BattleProfile battleProfile, LocalDateTime now) {
        ...
        return lineUpService.saveAll(shipLineUps);
    }
    ...
    public BattleProfile createShipLineUpWhenBattleForBase(BattleProfile battleProfile, LocalDateTime now) {
        ...
        for (EscortShip escortShip : escortShipList) {
            EscortShipSquad escortShipSquad = new EscortShipSquad().setEscortShip(escortShip).setAmount(escortShip.getAmount()).setLineUp(shipLineUp);
        ...
        motherShipList = motherShipList.stream().filter(motherShip -> motherShip.getStatus().equals(SeaActivityStatus.STANDBY)).sorted(
                Comparator.comparing(MotherShip::getCurrentHp).reversed()).collect(Collectors.toList());
        ...
        for (MotherShip motherShip : motherShipList) {
            ...
            shipLineUp1.setMotherShip(motherShip).setBattleProfile(battleProfile);
            shipLineUps.add(shipLineUp1);
            motherShip.setStatus(SeaActivityStatus.DEFENDING);

            motherShipServiceAsyncTask.sendHealingNotification(motherShip);
        }
    ...
    private BattleRoundSnapshot createBattleSnapshot(ShipLineUpBattle shipLineUpBattle, BattleRound battleRound) {
        ...
        // create snapshot
        BattleRoundSnapshot snapshot = new BattleRoundSnapshot().setBattleRound(battleRound).setAttackerModel(attacker).setDefenderModel(defender);
...
//        battleRoundRepository.save(battleRound);// check todo
        ...
        for (ShipLineUp lineUp : shipLineUps) {
            ...
            if (Objects.nonNull(battleProfile)) {
                if (!belligerentJoined.containsKey(battleProfile.getId())) {
                    Belligerent belligerent = new Belligerent().setBattleProfileId(battleProfile.getId()).setKosProfileId(
                            Objects.nonNull(battleProfile.getKosProfile()) ? battleProfile.getKosProfile().getId() : null); // case boss is null
    ...
    public void finalRoundBattle(Battle battle) {
        log.info("Final round battle {}, current round {}", battle.getId(), battle.getCurrentRound());
        ...
        RoundReport roundReport = new RoundReport().setRound(round).setBattleReport(round.getBattle().getBattleReport());
    ...
    private void validateBattleReport(List<BattleProfile> battleProfiles, BattleReport battleReport) {
        for (BattleProfile battleProfile : battleProfiles) {
            if (Objects.nonNull(battleProfile.getKosProfile()) && userBattleReportRepository.findByBattleIdAndKosProfileId(
                    battleReport.getBattle().getId(), battleProfile.getKosProfile().getId()).isEmpty()) {
        ...
        for (BattleProfile battleProfile : battle.getBattleProfiles()) {
            ...
            for (ShipLineUp lineUp : battleProfile.getShipLineUps()) {
                ...
                if (Objects.nonNull(lineUp.getMotherShip())) {
                    ...
                    // send message to client notify healing stop
    ...
    public void changeStatusBattle(BattleTask battleTask) {
        ...
        // ignore ended battle
        if (BattleStatus.END.equals(battle.getStatus()) || BattleStatus.CANCEL.equals(battle.getStatus())) {
        ...
        switch (battle.getStatus()) {
            case PROGRESS:
                ...
                if (checkWinBattleResult.getIsEnd()) {
                    ...
                    onBattleEnded(battle, checkWinBattleResult.getFactionTypeWin()); // warring
                } else {
                    log.info("Battle[{}]: Send new round to queue", battle.getId());
                    publisher.publishEvent(new CreateTaskBattleEvent(battle));
                }
    ...
    private Battle withdrawalLineUpAfterRound(Battle battle) {
        ...
        if (!idsLineupWithdrawal.isEmpty()) {
            List<ShipLineUp> shipLineUps = lineUpService.getLineupByIdIn(idsLineupWithdrawal).stream().filter(
                    lineUp -> Objects.nonNull(lineUp.getActiveMotherShip())).collect(Collectors.toList());
            if (!shipLineUps.isEmpty()) {
                withdrawListMotherShips(shipLineUps.stream().map(ShipLineUp::getMotherShip).collect(Collectors.toList()));
            }
        }
...
//        createBelligerent(defender);
    ...
    private Map<String, List<Ship>> toMapShip(List<ShipLineUp> shipLineUps) {
        ...
        for (ShipLineUp shipLineUp : shipLineUps) {
            ...
            if (motherShip != null && motherShip.getCurrentHp() > 0) {
                motherShip.setBattleProfileId(battleProfileId).setLineupId(shipLineUp.getId());
                motherShipList.add(motherShip);
            }
            if (Objects.isNull(shipLineUp.getEscortShipSquad())) {continue;}
            var escortShipSquads = shipLineUp.getEscortShipSquad().stream().filter(is -> (is.getAmount() > is.getKilled())).collect(
                    Collectors.toList());
            for (EscortShipSquad escortShipSquad : escortShipSquads) {
                escortShipSquad.setBattleProfileId(battleProfileId).setLineupId(shipLineUp.getId());
        ...
        for (BattleFiledRowsConfig rows : config.getRows()) {
            ...
            for (BattleFiledElementConfig elementConfig : rows.getElements()) {
                ...
                Long atk1Taken = Math.round(totalAtk1Taken * elementConfig.getPercentDamageTaken()
                                            + attackDamageReserveRows.getAtk1() * elementConfig.getPercentDamageTaken() / totalPercentDamageTakenRow
                                            + attackDamageReserveInRows.getAtk1());
                Long atk2Taken = Math.round(totalAtk2Taken * elementConfig.getPercentDamageTaken()
                                            + attackDamageReserveRows.getAtk2() * elementConfig.getPercentDamageTaken() / totalPercentDamageTakenRow
                ...
                if (listShipBattleVictim == null) {
                    attackDamageReserveInRows.setAtk1(atk1Taken).setAtk2(atk2Taken);
                } else {
                    AttackDamage attackDamage = new AttackDamage().setAtk1(atk1Taken).setAtk2(atk2Taken);
                    attackDamageReserveInRows = takeDamage(attackDamage, listShipBattleVictim);
                }

            }
            attackDamageReserveRows.setAtk1(attackDamageReserveInRows.getAtk1()).setAtk2(attackDamageReserveInRows.getAtk2());
        }
    }
    ...
    public AttackDamage takeDamageToShipBattle(AttackDamage attackDamage, BattleUnit shipBattle) {
        // for test
        shipBattle.setTotalAtk1Taken(attackDamage.getAtk1()).setTotalAtk2Taken(attackDamage.getAtk2());
        // end for test
        ...
        if (totalHp > currentHp) {
            ...
            attackDamage.setAtk1(0L).setAtk2(0L);
        } else {
            attackDamage.setAtk1(attackDamage.getAtk1() - atk1_).setAtk2(attackDamage.getAtk2() - atk2_);
            shipBattle.setCurrentHp(0L).setIsTookDamage(true);
        }
        shipBattle.setAtk1Redundant(attackDamage.getAtk1()).setAtk2Redundant(attackDamage.getAtk2());
        return attackDamage;
    }
    ...
    private Attack calculateTotalAtkAttack(Attack attack) {
        ...
        for (String s : setKeys) {
            ...
            for (BattleUnit shipBattle : shipBattles) {
                ...
                // for test
                shipBattle.setTotalAtk1Dealt(shipBattle.getAtk1() * shipBattle.getFighting()).setTotalAtk2Dealt(
                        shipBattle.getAtk2() * shipBattle.getFighting());
    ...
    public BattleFieldLineUpConfig chooseBattleField(Attack attacker, Attack defender) {
        ...
        defender.setTotalShip(totalShip - attacker.getTotalShip()).setBattleFieldInfo(battleFieldInfo);
    ...
    private CheckWinBattleResult checkFactionWinBattle(Battle battle) {
        ...
        if (attackerLose && defenderLose) {
            if (BattleType.ATTACK.equals(battle.getBattleType())) {
                return result.setIsEnd(true).setFactionTypeWin(FactionType.ATTACKER);
            } else {
                if (!isDisputeOccupied(battle)) {
                    result.setFactionTypeWin(FactionType.DEFENDER);
                }
                return result.setIsEnd(true);
            }
    ...
    private void updateShipToDatabase(Map<String, List<BattleUnit>> maps) {
        ...
        List<Long> idsMotherShip = shipBattles.stream().filter(v -> (v instanceof MotherShipBattle)).map(v -> (MotherShipBattle) v).map(
                MotherShipBattle::getMotherShipId).collect(Collectors.toList());
        List<Long> idsEscortShip = shipBattles.stream().filter(v -> (v instanceof EscortShipBattle)).map(v -> (EscortShipBattle) v).map(
                EscortShipBattle::getEscortShipSquadId).collect(Collectors.toList());
        ...
        for (MotherShip motherShip : motherShipList) {
            var optional = shipBattles.stream().filter(v -> (v instanceof MotherShipBattle)).map(v -> (MotherShipBattle) v).filter(
                    v -> v.getMotherShipId().equals(motherShip.getId())).findFirst();
        ...
        for (EscortShipSquad escortShipSquad : escortShipSquadList) {
            var optional = shipBattles.stream().filter(v -> (v instanceof EscortShipBattle)).map(v -> (EscortShipBattle) v).filter(
                    v -> v.getEscortShipSquadId().equals(escortShipSquad.getId())).findFirst();
        ...
        if (Objects.nonNull(factionTypeWin)) {
            switch (factionTypeWin) {
                case ATTACKER:
                    ...
                    battle.getBattleReport().setWinner(battle.getAttacker()).setLoser(battle.getDefender());
                    break;
                case DEFENDER:
                    battle.setWinner(battle.getDefender());
                    battle.getBattleReport().setWinner(battle.getDefender()).setLoser(battle.getAttacker());
                    break;
                default:
                    break;
            }
    ...
    private ShipLineUpBattle getListShipLineUpInBattle(Battle battle) {
        ...
        return new ShipLineUpBattle().setShipLineupAttacker(sortShipLineup(shipLineUpsAttacker)).setShipLineupDefender(
                sortShipLineup(shipLineUpsDefender));
    }
    ...
    private RoundReport updateRoundReport(RoundReport roundReport, Attack attacker, Attack defender) {
        ...
        // damage report
        roundReport.setAttackerDamageReport(createDamageReport(attacker)).setDefenderDamageReport(createDamageReport(defender));
    ...
    private DamageReport createDamageReport(Attack attack) {
        ...
        damageReport.setEscortShip(attack.getTotalEscortShipJoined()).setMotherShip(attack.getTotalMotherShipJoined()).setPhysicalAttack(
                            attack.getTotalAtk1Dealt()).setFirePower(attack.getTotalAtk2Dealt()).setTakenPhysicalAttack(attack.getTotalAtk1Taken())
                    .setTakenFirePower(attack.getTotalAtk2Taken()).setEscortShipLost(attack.getTotalEscortShipKilled()).setMotherShipLost(
                            attack.getTotalMotherShipLost());

        damageReport.setTotalMotherShipJoined(attack.getTotalMotherShipJoined()).setTotalEscortShipJoined(attack.getTotalEscortShipJoined())
                    .setTotalMotherShipLost(attack.getTotalMotherShipLost()).setTotalEscortShipKilled(attack.getTotalEscortShipKilled())
                    .setTotalHpMotherShipLost(attack.getTotalHpMotherShipLost()).setTotalAtk1Dealt(attack.getTotalAtk1Dealt()).setTotalAtk2Dealt(
                            attack.getTotalAtk2Dealt()).setTotalAtk1Taken(attack.getTotalAtk1Taken()).setTotalAtk2Taken(attack.getTotalAtk2Taken());
        return damageReport;
            ...
            if (!motherShip.isEmpty()) {
                ...
                for (BattleUnit shipBattle : motherShip) {
                    ...
                    if (motherShipBattle.getFighting() <= 0) {
                        ...
                        reverseMotherShipReport.setQuality(motherShipBattle.getQuality()).setBattleProfile(
                                maps.get(motherShipBattle.getBattleProfileId())).setModelId(motherShipBattle.getModelId()).setRoundReport(
                                roundReport);
                        reverseMotherShipReports.add(reverseMotherShipReport);
                    } else {
                        MotherShipReport motherShipReport = new MotherShipReport(factionType);

                        motherShipReport.setIndex(index++).setQuality(motherShipBattle.getQuality()).setBattleProfile(
                                                maps.get(shipBattle.getBattleProfileId())).setHpLost(
                                                motherShipBattle.getHpAfterBattle() - motherShipBattle.getCurrentHp()).setCurrentHp(motherShipBattle.getCurrentHp())
                                        .setMaxHp(shipBattle.getHp()).setModelId(motherShipBattle.getModelId()).setRoundReport(roundReport);
    ...
    private void createEscortShipReport(Attack attack, BattleRoundSnapshot roundSnapshotBefore, RoundReport roundReport, FactionType factionType) {
        ...
        for (var config : escortShipConfigs) {
            ...
            if (attack.getBattleFields().containsKey(key)) {
                ...
                escortShipReport.setEscortShipType(escortShipType).setEscortShipGroupName(shipGroupName).setLeft(totalShipFighting - totalShipKilled)
                                .setLost(totalShipKilled).setAdd(
                                        Objects.isNull(roundSnapshotBefore) ? 0 : totalShipFighting - escortShipFightingLiveBeforeRound.getOrDefault(key, 0L))
                                .setModelId(modelId).setRoundReport(roundReport);
                ...
                reverseEscortShipReport.setLeft(totalShipReverse).setEscortShipType(escortShipType).setEscortShipGroupName(shipGroupName)
                                       .setRoundReport(roundReport).setModelId(modelId);
    ...
    private void createAttackUserItem(Attack attack, Map<Long, BattleProfile> maps, RoundReport roundReport, FactionType factionType) {
        ...
        for (Entry<Long, Belligerent> entry : belligerentJoined.entrySet()) {
            ...
            roundUsedItem.setRoundReport(roundReport).setBattleProfile(maps.get(entry.getKey())).setItems(
                    getListItemById(entry.getValue().getItemIds())).setFaction(factionType);
    ...
    public void onBattleEnded(Battle battle, FactionType factionTypeWin) {
        ...
        switch (battle.getBattleType()) {
            case ATTACK:
                onBattleEndAttack(battle, factionTypeWin);
                break;
            case OCCUPY:
                onBattleEndOccupied(battle, factionTypeWin);
                break;
            default:
                break;
        }
    ...
    private void onBattleEndAttack(Battle battle, FactionType winner) {
        calculateReward(battle, winner);

        battleReportRepository.save(battle.getBattleReport());
        battle = battleRepository.save(battle);
        withdrawAllLineUp(battle);
        withdrawDefenseTroop(battle.getDefender());
    }

    private void onBattleEndOccupied(Battle battle, FactionType winner) {
        battleReportRepository.save(battle.getBattleReport());
        ...
        if (isDisputeOccupied(battle)) {
            if (Objects.nonNull(winner)) {
                if (FactionType.DEFENDER.equals(winner)) {
                    // withdraw Attacker
                    withdrawUserAllLineUp(battle.getAttacker().getKosProfile(), battle);
                    // withdraw defender if mother ship hp = 0
                    seaActivityService.anchorShip(
                            battle.getDefender().getShipLineUps().stream().map(s -> s.getActivity()).collect(Collectors.toList()), battleField);
                } else {
                    // withdraw defender
                    withdrawUserAllLineUp(battle.getDefender().getKosProfile(), battle);
                    // withdraw attacker if mother ship hp = 0
                    seaActivityService.anchorShip(
                            battle.getAttacker().getShipLineUps().stream().map(s -> s.getActivity()).collect(Collectors.toList()), battleField);
                    mapService.changeStatusElementToOccupied(battleField, battle.getAttacker().getKosProfile());
                }
            } else {
                withdrawAllLineUp(battle);
                // update seaActivity todo
                mapService.changeElementStatusToPeace(battleField);
            }
        } else {
            if (Objects.nonNull(winner) && FactionType.ATTACKER.equals(winner)) {
                seaActivityService.anchorShip(battle.getAttacker().getShipLineUps().stream().map(s -> s.getActivity()).collect(Collectors.toList()),
                                              battleField);
                mapService.changeStatusElementToOccupied(battleField, battle.getAttacker().getKosProfile());
                battleRepository.findByUserBaseIdAndStatus(battleField.getId(), List.of(BattleStatus.INIT)).forEach(
                        b -> b.cancel(BattleCancelReason.USER));
            } else {
                withdrawAllLineUp(battle);
            }
            withdrawDefenseTroop(battle.getDefender());
    ...
    private Boolean isDisputeOccupied(Battle battle) {
        assert battle.getBattleField() instanceof UserBase;
        UserBase userBase = (UserBase) battle.getBattleField();
        return !userBase.getKosProfile().getId().equals(battle.getDefender().getKosProfile().getId());
    }
    ...
    private void withdrawDefenseTroop(BattleProfile defender) {
        List<ShipLineUp> defenseLineUps = defender.getShipLineUps();
        for (int i = 0; i < defenseLineUps.size(); i++) {
            var defenseLineUp = defenseLineUps.get(i);
            lineUpService.onFinishMission(defenseLineUp);
        }
    }
    ...
    private BattleFinalReport createFinalReport(BattleFinalReport battleFinalReport, DamageReport damageReportRound) {
        battleFinalReport.setTotalAtk1(battleFinalReport.getTotalAtk1() + damageReportRound.getTotalAtk1Dealt()).setTotalAtk2(
                battleFinalReport.getTotalAtk2() + damageReportRound.getTotalAtk2Dealt()).setTakenAtk1(
                battleFinalReport.getTakenAtk1() + damageReportRound.getTotalAtk1Taken()).setTakenAtk2(
                battleFinalReport.getTakenAtk2() + damageReportRound.getTotalAtk2Taken()).setEscortShipLost(
                battleFinalReport.getEscortShipLost() + damageReportRound.getEscortShipLost()).setMotherShipDied(
                battleFinalReport.getMotherShipDied() + damageReportRound.getMotherShipLost()).setMotherShipHpLost(
                battleFinalReport.getMotherShipHpLost() + damageReportRound.getTotalHpMotherShipLost());
        return battleFinalReport;

    }

    private Collection<ShipLineUp> getUserLineUpsInBattle(Battle battle, KosProfile kosProfile) {
        var nullableBattleProfile = battle.getBattleProfiles().stream().filter(profile -> profile.getKosProfile().getId().equals(kosProfile.getId()))
                                          .findFirst();
    ...
    public void withdraw(KosProfile kosProfile, Battle battle, Collection<MotherShip> motherShips) {
        Set<MotherShip> motherShipsToWithdraw = new HashSet<>(motherShips);
        Collection<ShipLineUp> lineUpsInBattle = getUserLineUpsInBattle(battle, kosProfile);
        Set<MotherShip> motherShipsInBattle = lineUpsInBattle.stream().map(ShipLineUp::getMotherShip).collect(Collectors.toSet());
        motherShipsToWithdraw.retainAll(motherShipsInBattle);
        if (motherShips.size() == 0 || motherShipsToWithdraw.size() == lineUpsInBattle.size()) {
            boolean canWithdrawAll = withdrawUserAllLineUp(kosProfile, battle);
            if (battle.getAttacker().getKosProfile().getId().equals(kosProfile.getId())) {
                if (canWithdrawAll) {
                    try {
                        onBattleEnded(battle, FactionType.DEFENDER);
                    } catch (Exception e) {
                        battlePendingWithdrawalService.enqueueAttackerWithdrawalTask(battle.getId());
                    }
                } else {
                    battlePendingWithdrawalService.enqueueAttackerWithdrawalTask(battle.getId());
                }
        ...
        withdrawListMotherShips(motherShipsToWithdraw);
    }
...
//                                 .collect(Collectors.toList());
        UserBase userBase = (UserBase) battle.getBattleField();
        var participants = battle.getBattleProfiles().stream().filter(
                battleProfile -> !battleProfile.getKosProfile().getId().equals(userBase.getKosProfile().getId())).collect(Collectors.toList());
    ...
    /**
     * WARNING This logic for 1 vs 1 battle
     */
    private void calculateReward(Battle battle, FactionType winner) {
        log.info("Calculate reward {} {}", battle.getId(), battle.getStatus());
        ...
        var configs = configService.getBattleRewardConfig();

        // if attack winner calculate
        var battleReward = new BattleReward();
        var defender = battle.getDefender();
        // gold, stone & wood
        var weight = userLevelService.getLevel(battle.getAttacker().getKosProfile().getId()) - userLevelService.getLevel(
                battle.getDefender().getKosProfile().getId());

        BattleRewardItem config = null;
        for (BattleRewardItem item : configs.getItems()) {
            if (item.getStart() < weight && weight <= item.getEnd()) {
                config = item;
                break;
            }
        }
        ...
        var percent = config.getPercent();
        var numRelic = config.getNumRelic();
        var numQueen = config.getNumQueen();
        var numWeapon = config.getNumWeapon();
        var levelRelic = config.getLevelRelic();
        Double gold = defender.getKosProfile().getAssets().getGold() * percent,
                stone = defender.getKosProfile().getAssets().getStone() * percent,
                wood = defender.getKosProfile().getAssets().getWood() * percent;

        try {
            var protectPercent = vaultBuildingService.getBuildingInfo(new GetVaultBuildingInfo(defender.getKosProfile().getId(), true))
                                                     .getProtectPercent();
            gold = gold * (1 - protectPercent);
            stone = stone * (1 - protectPercent);
            wood = wood * (1 - protectPercent);
            assetsServiceAsyncTask.sendVaultProtectionNotification(defender.getKosProfile().getUser().getId(), protectPercent,
                                                                   new VaultProtectedResource().setGold(gold.longValue())
                                                                                               .setWood(wood.longValue())
                                                                                               .setStone(stone.longValue()));
        } catch (KOSException e) {
        ...
        defender.getKosProfile().getAssets().setGold(defender.getKosProfile().getAssets().getGold() - gold);
        defender.getKosProfile().getAssets().setStone(defender.getKosProfile().getAssets().getStone() - stone);
        defender.getKosProfile().getAssets().setWood(defender.getKosProfile().getAssets().getWood() - wood);

        battleReward.setBattleReport(battle.getBattleReport());
        battleReward.setWood(wood);
        battleReward.setWoodRemaining(wood);
        battleReward.setStone(stone);
        battleReward.setStoneRemaining(stone);
        battleReward.setGold(gold);
        battleReward.setGoldRemaining(gold);
        // relic, queen, weapon

        var relics = relicService.getRelicByLevelAndKosProfileId(levelRelic, defender.getKosProfile().getId());
        Collections.shuffle(relics);
        var pillagedRelic = relics.subList(0, Math.min(numRelic, relics.size()));
        for (Relic relic : pillagedRelic) {
            relic.setIsListing(false);
            relic.setCommunityBuilding(null);
        }
        battleReward.setRelics(pillagedRelic);

        var queens = queenService.getQueens(defender.getKosProfile().getId());
        Collections.shuffle(queens);
        var pillagedQueen = queens.subList(0, Math.min(numQueen, queens.size()));
        for (var queen : pillagedQueen) {
            queen.setQueenBuilding(null);
        }
        battleReward.setQueens(pillagedQueen);

        var weapons = weaponService.getWeaponNotEquipped(new GetWeaponsCommand().setKosProfileId(defender.getKosProfile().getId()));
        Collections.shuffle(weapons);
        var pillagedWeapons = weapons.subList(0, Math.min(numWeapon, weapons.size()));
        for (var weapon : pillagedWeapons) {
            weapon.setAssets(null);
        }
        battleReward.setWeapons(pillagedWeapons);

        queenService.saveAll(pillagedQueen);
        relicService.saveAll(pillagedRelic);
        weaponService.saveAll(pillagedWeapons);
        assetsService.save(defender.getKosProfile().getAssets());
        var reward = battleRewardService.save(battleReward);
        log.info("Reward {} {} {}", reward.getGold(), reward.getWood(), reward.getStone());
    ...
    public void sendBattleTaskToQueue(Battle battle) throws JsonProcessingException {
        ...
        BattleTimeConfigDetail battleTimeConfig = kosConfigService.getBattleTimeConfig().getAttack();
        var duration = 0L;
        switch (battle.getStatus()) {
            case BREAK:
        ...
        var task = new BattleTask().setBattleId(battle.getId()).setDefenderName(battle.getDefender().getType().name()).setBattleType(
        ...
        var mess = MessageBuilder.withBody(taskJson.getBytes()).andProperties(prop).build();
    ...
    public void joinBattle(Battle battle, BattleProfile battleProfileAlly, SeaActivity seaActivity) {
        ...
        if (battle.getStatus().equals(BattleStatus.PROGRESS)) {
            ...
            seaActivityService.updateCurrentActionStatus(seaActivity, battle.getTimeUpdateStatus()
                                                                            .plus(battleTimeConfig.getAttack().getBreakDuration(),
                                                                                  ChronoUnit.SECONDS), SeaActivityStatus.BREAK);
        }
    ...
    public boolean canJoinCombat(SeaActivity activity, SeaElement element) {
        MoveSession moveSession = activity.getActiveMoveSession();
        Battle battleOnTarget = element.getBattle();
        if (Objects.isNull(battleOnTarget)) {
            return false;
        }
        if (battleOnTarget.getStatus().equals(BattleStatus.END)) {
            return false;
        }
        if (Objects.isNull(moveSession.getBattleId()) || !moveSession.getBattleId().equals(battleOnTarget.getId())) {
            return false;
        }
        MissionType missionType = moveSession.getMissionType();
        BattleType battleType = battleOnTarget.getBattleType();
        if (missionType.equals(MissionType.OCCUPY) && (battleType.equals(BattleType.OCCUPY) || battleType.equals(BattleType.LIBERATE))) {
            return true;
        }
        if (missionType.equals(MissionType.ATTACK) && battleType.equals(BattleType.ATTACK)) {
            return true;
        }
        return false;
    }
