...
@AllArgsConstructor
@Slf4j
public class BattleMiningService implements BattleHandler{
    ...
    @Transactional
    public Battle startBattle(Battle battle, SeaElement battleField, SeaActivity attackerActivity, SeaActivity defenderActivity) {
        ...
        updateTypeResource(battle, battleField);
        // update shipLineUp for battle
        updateShipLineUpForBattle(battle.getAttacker(), attackerActivity, now);
        updateShipLineUpForBattle(battle.getDefender(), defenderActivity, now);

        List<BattleProfile> battleProfiles = new ArrayList<>();
        battleProfiles.add(battle.getDefender());
        battleProfiles.add(battle.getAttacker());
        battle.setBattleProfiles(battleProfiles);
        battle.getBattleReport().setJoiners(battleProfiles);
        beforeBattleProgress(battle);
        battleRepository.save(battle);
        mapService.saveOrUpdateElement(new SaveOrUpdateElementCommand(battleField));
        updateAllBattleActivityTimer(battle, LocalDateTime.now().plus(kosConfigService.getBattleTimeConfig().getMine().getProgressDuration(),
                                                                      ChronoUnit.SECONDS), SeaActivityStatus.ATTACKING);
        return battle;
    }

    private void updateTypeResource(Battle battle, SeaElement battleField) {
        if (BattleType.MINE.equals(battle.getBattleType()) && Objects.nonNull(battleField)) {
            if (battleField instanceof ResourceIsland) {
                battle.getBattleReport().setResourceType(((ResourceIsland) battleField).getResourceType());
            }
        }
    }

    private void updateShipLineUpForBattle(BattleProfile battleProfile, SeaActivity activity, LocalDateTime now) {
        // check if need todo
        List<ShipLineUp> shipLineUps = new ArrayList<>();
        ShipLineUp shipLineUp = activity.getLineUp();
    ...
    private void beforeBattleProgress(Battle battle) {
        ...
        CheckWinBattleResult checkWinBattleResult = checkFactionWinBattle(snapshot.getAttackerModel(), snapshot.getDefenderModel(), battle, false);
...
//        battleRoundRepository.save(battleRound);// check todo
    ...
    public void finalRoundBattle(Battle battle) {
        ...
        // check end battle
        CheckWinBattleResult checkWinBattleResult = checkFactionWinBattle(attacker, defender, battle, true);
        if (checkWinBattleResult.getIsEnd()) {
...
//        battle.setStatus(BattleStatus.BREAK);
    ...
    private void updateAllBattleActivityTimer(Battle battle, LocalDateTime timeEnd, SeaActivityStatus status) {
        for (BattleProfile battleProfile : battle.getBattleProfiles()) {
            ...
            for (SeaActivity activity : activities) {
                if (battleProfile.getKosProfile().getId().equals(
                        battle.getDefender().getKosProfile().getId())) {
                    seaActivityService.updateCurrentActionStatus(activity, timeEnd, SeaActivityStatus.DEFENDING);
                } else {
    ...
    @Transactional
    public void changeStatusBattle(BattleTask battleTask) throws JsonProcessingException {
        ...
        log.info("Battle:{} change status : current status : {}, current round {}", battleId, battle.getStatus(), battle.getCurrentRound());
        switch (battle.getStatus()) {
            case PROGRESS:
                ...
                CheckWinBattleResult checkWinBattleResult = checkWithdrawAllAfterRound(battle);
                if (checkWinBattleResult.getIsEnd()) {
                    onBattleEnded(battle, checkWinBattleResult.getFactionTypeWin());
                    withdrawalLineUpAfterRound(battle);
                } else {
                    beforeBattleProgress(battle);
                    updateAllBattleActivityTimer(battle,
                                                 LocalDateTime.now().plus(kosConfigService.getBattleTimeConfig().getMine().getProgressDuration(),
                                                                          ChronoUnit.SECONDS), SeaActivityStatus.ATTACKING);
                }
                break;
            default:
    ...
    public CheckWinBattleResult checkWithdrawAllAfterRound(Battle battle) {
        CheckWinBattleResult result = new CheckWinBattleResult();
        ...
        if (!idsLineupWithdrawal.isEmpty()) {
            ...
            List<ShipLineUp> attackerWithdrawal = shipLineUps.stream().filter(
                                                                     shipLineUp -> FactionType.ATTACKER.equals(shipLineUp.getBattleProfile().getFaction()))
                                                             .collect(Collectors.toList());
            List<ShipLineUp> defenderWithdrawal = shipLineUps.stream().filter(
                                                                     shipLineUp -> FactionType.DEFENDER.equals(shipLineUp.getBattleProfile().getFaction()))
                                                             .collect(Collectors.toList());
            if (!attackerWithdrawal.isEmpty() && !defenderWithdrawal.isEmpty()) {
                return result.setIsEnd(true);
            } else if (!attackerWithdrawal.isEmpty()) {
                return result.setIsEnd(true).setFactionTypeWin(FactionType.DEFENDER);
            } else if (!defenderWithdrawal.isEmpty()) {
    ...
    private void withdrawalLineUpAfterRound(Battle battle) {
        if (!battle.getStatus().equals(BattleStatus.END)) {
            List<NextRoundWithdrawal> nextRoundWithdrawals = nextRoundWithdrawalRepo.findByBattleId(battle.getId());
            List<Long> idsLineupWithdrawal = nextRoundWithdrawals.stream()
                                                                 .map(NextRoundWithdrawal::getLineUpId)
                                                                 .distinct()
                                                                 .collect(Collectors.toList());
            if (!idsLineupWithdrawal.isEmpty()) {
                List<ShipLineUp> shipLineUps = lineUpService.getLineupByIdIn(idsLineupWithdrawal)
                                                            .stream()
                                                            .filter(lineUp -> Objects.nonNull(lineUp.getActiveMotherShip()))
                                                            .collect(Collectors.toList());
...
//        createBelligerent(defender);
    ...
    private CheckWinBattleResult checkFactionWinBattle(Attack attacker, Attack defender, Battle battle, Boolean checkShipWaiting) {
        List<ShipLineUp> shipLineUpsAttacker = new ArrayList<>();
        List<ShipLineUp> shipLineUpsDefender = new ArrayList<>();
        if (checkShipWaiting) {
            log.info("Battle:{},checkShipWaiting, round: {}, status:{}, timeUpdateStatus:{}", battle.getId(), battle.getCurrentRound(),
                     battle.getStatus(), battle.getTimeUpdateStatus());
            shipLineUpsAttacker = lineUpService.getListShipLineupWaitingInBattle(battle.getId(), FactionType.ATTACKER, battle.getTimeUpdateStatus());
            shipLineUpsDefender = lineUpService.getListShipLineupWaitingInBattle(battle.getId(), FactionType.DEFENDER, battle.getTimeUpdateStatus());
        }
        Boolean attackerLose = checkLostBattle(attacker.getBattleFields(), shipLineUpsAttacker, battle.getCheckResult().getAttackerCheckResult());
        Boolean defenderLose = checkLostBattle(defender.getBattleFields(), shipLineUpsDefender, battle.getCheckResult().getDefenderCheckResult());
    ...
    private Boolean checkLostBattle(Map<String, List<BattleUnit>> maps, List<ShipLineUp> shipLineUps, FlatCheckResultBattle flatCheckResultBattle) {
        switch (flatCheckResultBattle) {
            ...
            case ALL:
                ...
                for (String s : keySets) {
                    List<? extends BattleUnit> shipBattleList = maps.get(s);
                    for (BattleUnit shipBattle : shipBattleList) {
                        if (shipBattle.getAmount() > shipBattle.getKilled()) {
                            return false;
                        }
                    }
                }
    ...
    public void onBattleEnded(Battle battle, FactionType factionTypeWin) {
        ...
        calculateReward(battle, factionTypeWin);
        ...
        battle = battleRepository.save(battle);
        if (Objects.nonNull(factionTypeWin)) {
            withdrawUserAllLineUp(battle.getLoser().getKosProfile(), battle);
            if (battle.getWinner().getShipLineUps().size() > 0) {
                resourceMiningService.startMiningSession(battle.getWinner().getShipLineUps().get(0).getActivity(),
                                                         (ResourceIsland) battle.getBattleField());
            }
        } else {
            for (BattleProfile battleProfile : battle.getBattleProfiles()) {
        ...
        log.info("Update sea map element : mining");
    ...
    public void sendBattleTaskToQueue(Battle battle) throws JsonProcessingException {
        ...
        switch (battle.getStatus()) {
            case BREAK:
                log.info("WARNING, Battle:{} , combat mining have BREAK", battle.getId());
                break;
            case PROGRESS:
                duration = kosConfigService.getBattleTimeConfig().getMine().getProgressDuration() * 1000;
                break;
        }
        ...
        if (battle.getStatus().equals(BattleStatus.PROGRESS)) {
            ...
            seaActivityService.updateCurrentActionStatus(seaActivity,
                                                         battle.getTimeUpdateStatus()
                                                               .plus(battleTimeConfig.getMine().getBreakDuration(), ChronoUnit.SECONDS),
