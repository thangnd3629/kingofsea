...
public class AppStartupRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        var messageStr = String.format("%s Cron Job is started ", Arrays.toString(environment.getActiveProfiles()));
        log.info(messageStr);
    ...
    public void onDestroy() throws IOException {
        var messageStr = String.format("%s Cron Job is stoped", Arrays.toString(environment.getActiveProfiles()));
        log.info(messageStr);
        var version = FileUtils.readFileToString(new File("./VERSION.txt"), Charsets.UTF_8);
        var message = new SendMessage("-628648424", messageStr + "\nVersion: " + version);
        telegramBot.execute(message);
